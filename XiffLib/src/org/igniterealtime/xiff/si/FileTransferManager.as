...
package org.igniterealtime.xiff.si
{
	...
	import org.igniterealtime.xiff.data.forms.enum.FormFieldType;
	import org.igniterealtime.xiff.data.forms.enum.FormType;
	import org.igniterealtime.xiff.data.message.OutOfBoundDataExtension;
	import org.igniterealtime.xiff.data.si.FileTransferExtension;
	import org.igniterealtime.xiff.data.si.StreamInitiationExtension;
	import org.igniterealtime.xiff.data.stream.IBBCloseExtension;
	import org.igniterealtime.xiff.data.stream.IBBDataExtension;
	import org.igniterealtime.xiff.data.stream.IBBExtension;
	import org.igniterealtime.xiff.data.stream.IBBOpenExtension;
	import org.igniterealtime.xiff.events.FileTransferEvent;
	...
	public class FileTransferManager extends EventDispatcher
	{
		...
		private function sendOpen_callback(iq:IQ):void
		{
			...
			*/
			...
			_outGoingBase64 = Base64.encodeByteArray(_outGoingData );
			sendChunk(iq.from);
		}
		...
		private function sendChunk(to:EscapedJID):void
		{
			...
			if(_outGoingBase64.length > _chunkSize){
				var dataExt:IBBDataExtension = new IBBDataExtension();
				dataExt.sid = "letsdoit";
				dataExt.seq = _sequenceOut;
				if((_outGoingBase64.length > _chunkSize * _sequenceOut)){
					dataExt.data = _outGoingBase64.substr(_chunkSize * _sequenceOut, _chunkSize);
				}else{
					//dataExt.data = _outGoingBase64.substr(_chunkSize * (_sequenceOut-1), (_outGoingBase64.length - (_chunkSize * (_sequenceOut-1))));
		...
		{
			trace(Number(_outGoingBase64.length) - Number(_chunkSize * (_sequenceOut-1)));
			if (Number(_outGoingBase64.length) - Number(_chunkSize * (_sequenceOut-1)) > 0)
			{
				sendChunk(iq.from);
			}
			else
			{
				var iq:IQ = new IQ(iq.from, IQ.TYPE_SET, null, sendClose, sendChunk_errorCallback);
				iq.addExtension(new IBBCloseExtension());
				_connection.send(iq);
		...
		private function sendClose(iq:IQ):void{
			trace(iq);
		}
